...
import { Calendar, Award, Lock, Edit, CheckCircle2, ShieldCheck, Target, Zap } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const [userName, setUserName] = useState('TradeVault User');
  const [userEmail, setUserEmail] = useState('');
  const [avatarUrl, setAvatarUrl] = useState('');
  const [memberSince, setMemberSince] = useState('Aug 2026');
  ...
  useEffect(() => {
    async function loadUser() {
      ...
      if (user) {
        ...
        const saved = localStorage.getItem('tradevault_user_preferences');
        if (saved) {
  ...
  const achievements = useMemo(() => [
    ...
    { title: 'High Win Rate (>55%)', desc: 'Demonstrated edge in setups.', unlocked: stats.win_rate >= 55 },
    { title: '10+ Trades Logged', desc: 'Building consistent journaling habits.', unlocked: trades.length >= 10 },
    { title: 'Funded Account Added', desc: 'Configured trading capital.', unlocked: accounts.length > 0 },
          ...
          <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
            <Avatar className="w-24 h-24 border-4 border-background ring-2 ring-primary/20">
              ...
              <AvatarFallback className="text-2xl font-bold bg-primary/10 text-primary">
                {userName.slice(0, 2).toUpperCase() || 'TV'}
              </AvatarFallback>
            </Avatar>
            <div className="flex-1">
              <h1 className="text-3xl font-bold">{userName}</h1>
              <p className="text-muted-foreground flex items-center justify-center md:justify-start gap-2 mt-1">
                {userEmail || 'trader@tradevault.com'} <span className="text-xs text-muted-foreground/50">•</span> 
                <Calendar className="w-3.5 h-3.5" /> Member since {memberSince}
              </p>
            </div>
            <Link href="/settings">
              <Button variant="outline" className="gap-2">
                <Edit className="w-4 h-4" /> Edit Profile in Settings
              </Button>
            </Link>
          </div>
        </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="md:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Visual representation of your portfolio equity over closed trades.</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Breakdown by volume and profitability</CardDescription>
          </CardHeader>
